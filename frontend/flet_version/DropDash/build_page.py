            ...
            content=Column(
                controls=[
                    Container(
                        ...
                        content=Text("Log In", weight="bold", size=50),
                        left=0,
                        bottom=670,
                    ),
                    Container(
...
class AirdropUploadPage(BuildPage):
    def __init__(self, page: Page, post_airdrop):
        ...
        self.selected_file = None  # Initialize selected_file to avoid AttributeError

        # Input fields for Airdrop details
        self.name_input = TextField(label="Airdrop Name", width=400)
        self.description_input = TextField(
        ...
        self.category = Dropdown(
            ...
            options=[
                ...
                dropdown.Option("Retroactive"),
                dropdown.Option("Mining"),
                dropdown.Option("NFT Airdrops"),
                dropdown.Option("SocialFi"),
                dropdown.Option("GameFi"),
                dropdown.Option("General Airdrop"),
                dropdown.Option("Promotional Airdrop"),
                dropdown.Option("Others"),
            ],
            width=200,
        )
        self.image_upload_input = FilePicker(on_result=self.on_file_result)
        self.image_preview = Image(src="", width=100, height=100)
        self.amount_raised = TextField(
            label="Enter Amount Raised",
            keyboard_type="number",
        )

        # Initialize CupertinoDatePicker
        self.cupertino_date_picker = CupertinoDatePicker(
            date_picker_mode=CupertinoDatePickerMode.DATE,
            on_change=self.handle_date_change,
        )

        # Text field to display selected date
        self.airdrop_start_date = Text("")

        self.select_airdrop_start_button = ElevatedButton(
            text="Select Airdrop Start Date",
        ...
        self.page.overlay.append(self.image_upload_input)

        # Submit button
        self.submit_button = ElevatedButton(
            text="Create Airdrop", on_click=self.handle_airdrop_upload
        )

        # Layout the form
        self.page_content = Column(
            [
                Text("Create New Airdrop", size=30, weight="bold"),
                self.name_input,
                Row(
                    [
                        ElevatedButton("Upload Image", on_click=self.pick_image),
                        self.image_preview,
                    ]
                ),
                ...
                self.category,
                self.amount_raised,
                Row([self.select_airdrop_start_button, self.airdrop_start_date]),
                self.submit_button,
            ],
    ...
    def on_file_result(self, e: FilePickerResultEvent):
        if e.files:
            ...
            if any(self.selected_file.name.endswith(ext) for ext in valid_extensions):
                self.image_preview.src = self.selected_file.path
                self.image_preview.update()
                snack_bar = SnackBar(
                    Text(f"Selected: {self.selected_file.name}"), open=True
                )
            else:
                snack_bar = SnackBar(
    ...
    async def handle_airdrop_upload(self, e: ControlEvent):
        ...
        image_url = self.selected_file.path if self.selected_file else None
        description = self.description_input.value
        category = self.category.value
        amount_raised = self.amount_raised.value
        airdrop_start_date = self.airdrop_start_date.value
